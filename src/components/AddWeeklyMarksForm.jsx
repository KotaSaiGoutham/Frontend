...
import { MuiInput, MuiButton } from "./MuiCustomFormFields"; // Removed MuiDatePicker as it's not used here
...
const AddWeeklyMarksModal = ({
  ...
  console.log("AddWeeklyMarksModal - programType (on render):", programType);
  ...
  const handleSubmit = async () => {
    ...
    console.log("Current Scheme for", programType, ":", currentScheme);
    console.log("Marks Input state:", marksInput);
    ...
    if (currentScheme) {
      ...
      for (const subject in currentScheme) {
        ...
        if (enteredMarkString === "") {
          setError(`**ERROR:** Please enter marks for ${formatSubjectName(subject)}.`);
        ...
        if (isNaN(markValue)) {
          setError(`**ERROR:** Invalid marks entered for ${formatSubjectName(subject)}. Please enter a number.`);
          hasValidationError = true;
          break;
        }
        if (markValue < 0) {
          setError(`**ERROR:** Marks for ${formatSubjectName(subject)} cannot be negative.`);
          hasValidationError = true;
          break;
        }
        if (markValue > maxMark) {
          setError(`**ERROR:** ${formatSubjectName(subject)} marks (${markValue}) cannot exceed Max Marks of ${maxMark}. Please correct this.`);
          hasValidationError = true;
          break;
        }
      ...
      if (customFields.length === 0) {
        ...
        for (const field of customFields) {
          ...
          if (enteredMarkString === "" || enteredMaxMarkString === "") {
            setError(`**ERROR:** Please enter both marks obtained and max marks for '${field.subjectName || 'a custom subject'}'`);
            hasValidationError = true;
            break;
          }
          ...
          if (isNaN(markValue) || isNaN(maxMarkValue)) {
            setError(`**ERROR:** Invalid numbers entered for marks or max marks for '${field.subjectName || 'a custom subject'}'`);
            hasValidationError = true;
            break;
          }
          if (markValue < 0 || maxMarkValue < 0) {
            setError(`**ERROR:** Marks or max marks for '${field.subjectName || 'a custom subject'}' cannot be negative.`);
            hasValidationError = true;
            break;
          }
          if (maxMarkValue === 0) {
            setError(`**ERROR:** Max Marks for '${field.subjectName || 'a custom subject'}' cannot be zero.`);
            hasValidationError = true;
            break;
          }
          if (markValue > maxMarkValue) {
            setError(`**ERROR:** Marks obtained (${markValue}) for '${field.subjectName || 'a custom subject'}' cannot exceed Max Marks (${maxMarkValue}). Please correct this.`);
            hasValidationError = true;
            break;
          }
    ...
    console.log("Sending newMark payload (after validation):", newMark);

    try {
      const response = await fetch(
        `http://localhost:5000/api/data/students/${studentId}/marks/weekly`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || `Failed to add marks for ${programType}`
        );
      }
      ...
      // Set error if it's an API-related error
      // This will overwrite any validation error, which is desired for API errors
      setError(err.message || "An unexpected error occurred during submission.");
      console.error("Error adding weekly marks:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderMarkInputs = () => {
    const scheme = MARK_SCHEMES[programType];
    console.log("renderMarkInputs - scheme:", scheme); // Debugging scheme being used
    ...
    if (scheme) {
      ...
      return (
        <Stack spacing={2}> {/* Use Stack here for consistent spacing and layout */}
          {customFields.map((field, index) => (
            <div key={field.id} className="custom-mark-field-group" style={{
                display: 'flex',
