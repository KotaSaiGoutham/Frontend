...
const AddTimetablePage = () => {
  ...
  const selectedFaculty = employees.find(emp => String(emp.id) === formData.facultyId);
  const facultyName = selectedFaculty ? selectedFaculty.name : "Unknown Faculty";

  const selectedStudent = students.find(stu => String(stu.id) === formData.student);
  ...
  try {
    if (timetableToEdit) {
      if (timetableToEdit.isAutoGenerated) {
        // Calculate classDateTime & dateTimeISO for Firestore
        let classDateTimeObj = null;
        ...
        if (formData.fromTime && isValid(formData.fromTime) && isValid(dateObject)) {
          ...
          combinedDateTime.setHours(formData.fromTime.getHours());
          combinedDateTime.setMinutes(formData.fromTime.getMinutes());
          combinedDateTime.setSeconds(0);
          combinedDateTime.setMilliseconds(0);

          classDateTimeObj = {
            _seconds: Math.floor(combinedDateTime.getTime() / 1000),
            _nanoseconds: combinedDateTime.getMilliseconds() * 1_000_000,
          };

          const offsetMinutes = combinedDateTime.getTimezoneOffset();
          const offsetSign = offsetMinutes > 0 ? "-" : "+";
          const offsetHours = String(Math.floor(Math.abs(offsetMinutes) / 60)).padStart(2, "0");
          const offsetMins = String(Math.abs(offsetMinutes) % 60).padStart(2, "0");
          dateTimeISOWithOffset = `${format(combinedDateTime, "yyyy-MM-dd'T'HH:mm:ss")}${offsetSign}${offsetHours}:${offsetMins}`;
        }

        const updatedAutoGeneratedPayload = {
          ...timetableToEdit,
          ...basePayload,
          classDateTime: classDateTimeObj,
          dateTimeISO: dateTimeISOWithOffset,
          isAutoGenerated: true,
          isAutoGeneratedInDb: true,
          topicId: topicOptions.find(opt => opt.topic === formData.topic)?.id || null,
        };

        await dispatch(updateAutoTimetableEntry(updatedAutoGeneratedPayload));
      } else {
        // Regular update for manual entry
    ...
    setSubmitMessage({ type: "success", message: `Timetable entry ${timetableToEdit ? "updated" : "added"} successfully!` });
          ...
          <form onSubmit={handleSubmit} className="ats-form">
            ...
            <div className="add-student-button-group">
              <button type="submit" className="add-student-primary-button">
                <FaSave />{" "}
                {!!timetableToEdit ? "Update Class" : "Schedule Class"}
              </button>
              <button
                type="button"
                onClick={() => navigate("/timetable")}
