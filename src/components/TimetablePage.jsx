...
// --- NEW STATE TO CONTROL AUTO-GENERATION ---
  ...
  const [hasAttemptedAutoGenerateSession, setHasAttemptedAutoGenerateSession] = useState(false);
 ...
 const generateTimetablesForStudents = useCallback(async () => {
    ...
    if (
      ...
      if (!user || !user.id) setSnackbarMessage("User data not loaded or not authorized. Cannot auto-generate.");
      else if (students.length === 0) setSnackbarMessage("No students found to generate timetables for.");
    ...
    if (autoTimetablesHasSavedToday) {
      console.log(`Auto-generated timetables for today already exist in DB for this user. Skipping regeneration.`);
      setAutoGenerateErrorMsg(null);
      setIsGeneratingAuto(false);
      setSnackbarSeverity("info");
      setSnackbarMessage("Daily auto-timetables already generated and saved for today.");
      setSnackbarOpen(true);
    ...
    const relevantStudents = students
      .filter(
        (student) => canAccessAll || student.Subject?.trim() === currentUserSubject
      )
...
console.log("sdfsdf",autoTimetables)
      try {
        ...
        setSnackbarMessage(`Successfully generated and saved ${finalGeneratedItems.length} timetables for today!`);
        setSnackbarOpen(true);
        // setHasAttemptedAutoGenerateSession(true); // Mark as attempted for this session
      } catch (error) {
        setAutoGenerateErrorMsg(`Failed to save auto-generated timetables: ${error.message}`);
        setSnackbarSeverity("error");
        setSnackbarMessage(`Failed to save auto-generated timetables: ${error.message}`);
        setSnackbarOpen(true);
      }
    } else {
      setSnackbarSeverity("info");
      setSnackbarMessage("No new timetables could be auto-generated for today.");
      setSnackbarOpen(true);
    }
...
// TimetablePage.jsx
 ...
 // This handles initial GET requests, preventing them from doubling in StrictMode.
    useDidMountEffect(() => {
        console.log("Effect 1: Running initial data fetches (single call in StrictMode dev).");
        dispatch(fetchUpcomingClasses());
        dispatch(fetchStudents());
        if (user && user.id) {
            console.log("Effect 1: Dispatching fetchAutoTimetablesForToday for user:", user.id);
            dispatch(fetchAutoTimetablesForToday());
        } else {
            console.warn("Effect 1: User or user.id is not available yet for fetching auto timetables. Will retry if user becomes available.");
        }
    }, [dispatch, user]);
    ...
    useDidMountEffect(() => { // <--- CHANGED from useEffect to useDidMountEffect
        console.log("Effect 2: Checking conditions for auto-generation (single call in StrictMode dev). State:", {
        ...
        if (!user || !user.id || studentsLoading || classesLoading || !students || students.length === 0) {
            console.log("Effect 2: Data not ready or user not logged in. Skipping generation trigger.");
            return;
        }

        // 2. CRITICAL: Wait for the initial fetch for autoTimetables to complete
        if (autoTimetablesLoading) {
            console.log("Effect 2: Auto timetables are still loading. Waiting for fetch to complete.");
            return;
        }
        ...
        if (autoTimetablesHasSavedToday) {
            console.log("Effect 2: Auto timetables already exist for today. No generation needed.");
        ...
        if (hasAttemptedAutoGenerateSession) {
            console.log("Effect 2: Already attempted auto-generation in this session. Skipping re-trigger.");
            return;
        }

        console.log("Effect 2: Conditions met for auto-generation. Calling generateTimetablesForStudents().");
        ...
        dispatch, user, students, studentsLoading, classesLoading,
        autoTimetablesLoading, autoTimetablesHasSavedToday, generateTimetablesForStudents,
 ...
 const dateA = getDateFromTimetableItem(a);
  ...
  const handleDeleteClick = (item) => {
    ...
    setTimetableToDelete({ ...item, isAutoGeneratedInDb: isFromAutoTimetables });
 ...
 const isLoading = classesLoading || studentsLoading || autoTimetablesLoading || isGeneratingAuto || isDeleting;
    ...
    >
                  ...
                  {combinedAndFilteredTimetables.map((item, index) => {
                    ...
                    switch (rowStatus) {
                      ...
                      case "pastDay":
                        rowSx = {
                          background:
                            "linear-gradient(90deg, #f8f8f8 0%, #eeeeee 100%)",
                          opacity: 0.55,
                          pointerEvents: "none",
                          "& > td": {
                            color: "#b0b0b0",
                        ...
                        disableActions = true;
                        tooltipEditTitle = "Past class (edit disabled)";
                        tooltipDeleteTitle = "Past class (delete disabled)";
                        break;
                      ...
                      default:
                        rowSx = {
                          background:
                            "linear-gradient(to right, #ffffff, #f7faff)",
                            ...
                            {item.isAutoGenerated ? (
                              ...
                              <>
                                ...
                                <Typography
                                  variant="caption"
                                  sx={{
                                            background: "linear-gradient(to right, #bbdefb, #64b5f6)", // Blue
     ...
     <Dialog
        ...
        >
          ...
          <DialogActions>
            <Button onClick={() => setOpenDeleteConfirm(false)} disabled={isDeleting}>
              Cancel
            </Button>
